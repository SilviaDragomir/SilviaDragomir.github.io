...
For the campaign promoting the club, customers were put randomly into three groups - the first group received a low quality, low cost mailer, the second group received a high quality, high cost mailer, and the third group were a control group, receiving no mailer at all. This is the design for the A/B Test.
...
As a requirement of the Chi-Square Test For Independence, we aggregated this data down to a 2x2 matrix as a contingency table for *signup_flag* by *mailer_type* and fed this into the algorithm (using the *scipy* library) to calculate the Chi-Square Statistic, p-value, Degrees of Freedom, and expected frequencies.
...
Based upon our observed frequencies, we get:
...
An A/B Test can be described as a randomised experiment containing two groups, A and B, that receive different experiences. Within an A/B Test, we look to understand and measure the response of each group - and the information from this helps drive future business decisions.
...
There are many different scenarios we can run Hypothesis Tests on, and they all have slightly different techniques and formulas - however they all have some shared, fundamental steps and logic that support how they work.
...
There are many different types of Hypothesis Tests, each of which is appropriate for use in differing scenarios - depending on a) the type of data that you are looking to test and b) the question that you are asking of that data.
...
The *observed frequencies* are the true values that we have seen.
...
In the code below we code these in explcitly and clearly so we can utilise them later to explain the results.  We specify the common Acceptance Criteria value of 0.05.
...
**Note** When applying the Chi-Square Test above, we use the parameter *correction = False* which means that the *Yate's Correction* is not applied, our test having a rather large sample of data and in this case the recommandation is not to apply the Yates Correction. However the Python code was run with both alternatives, without and with Yates corretion to prevent overestimation of statistical significance and the results were the same. The interpretation in both cases was *Not significant*.
...
Our results here also do not say that there *definitely there is no difference between the two mailers* - we are only advising that we should not make any rigid conclusions *at this point*.  
